...
(() => {
  ...
  // src/utils/onlineshop/swiper/returnAttributes.ts
  function returnNumberByAttribute(el, attributeName) {
    const numberValue = el.getAttribute(attributeName);
    if (numberValue)
      return getNumberValueByAttribute(numberValue);
    return;
  }
  function returnStringByAtrtribute(el, attributeName) {
    const stringValue = el.getAttribute(attributeName);
    if (stringValue)
      return getStringValueByAttribute(stringValue);
    return;
  }
  function returnBooleanValueByAttribute(el, attributeName) {
    const booleanValue = el.getAttribute(attributeName);
    if (booleanValue)
      return getBooleanValueByAttribute(booleanValue);
    return;
  }
  ...
  function getAllSwiperAtributeValues(el, attributesList) {
    ...
    const { booleanValues, stringValues, numberValues } = attributesList;
    function assignObjectEntriesToNewObject(iterateObject, assignObject, variableType) {
      switch (variableType) {
        case "string":
          assignAttributeValues(iterateObject, assignObject, el, returnStringByAtrtribute);
          break;
        case "number":
          assignAttributeValues(iterateObject, assignObject, el, returnNumberByAttribute);
          break;
        case "boolean":
    ...
    assignObjectEntriesToNewObject(stringValues, attributeValues, "string");
    assignObjectEntriesToNewObject(numberValues, attributeValues, "number");
  ...
  var mountSliders = () => {
    ...
    sliderWrapperEl.forEach((el) => {
      ...
      const swiper = new Swiper(swiperEl, {
        ...
        freeMode: freemodeValue ? freemodeValue : false,
        ...
        breakpoints: {
          ...
          992: {
            ...
            allowTouchMove: touchMoveDesktop ? touchMoveDesktop : false
          },
          1440: {
