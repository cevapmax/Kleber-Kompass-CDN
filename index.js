...
(() => {
  ...
  init_live_reload();
  var getslidesperViewByAttribute = (elementAtrribute) => {
    if (!elementAtrribute) {
  ...
  var getSpaceBetweenByAtrtribute = (elementAtrribute) => {
    if (!elementAtrribute) {
      elementAtrribute = void 0;
    }
    return elementAtrribute;
  ...
  var mountSliders = () => {
    ...
    sliderWrapperEl.forEach((el) => {
      ...
      let freemodeValue = el.getAttribute("freemode");
      freemodeValue = getBooleanValueByAttribute(freemodeValue);
      let loopValue = el.getAttribute("loop-mode");
      loopValue = getBooleanValueByAttribute(loopValue);
      let isAlternate = el.getAttribute("alternate");
      isAlternate = getBooleanValueByAttribute(isAlternate);
      let touchMoveDesktop = el.getAttribute("allow-touch-desktop");
      touchMoveDesktop = getBooleanValueByAttribute(touchMoveDesktop);
      let spaceBetweenBaseValue = el.getAttribute("space-between-base");
      spaceBetweenBaseValue = getSpaceBetweenByAtrtribute(spaceBetweenBaseValue);
      let spaceBetweenMobileValue = el.getAttribute("space-between-mobile");
      spaceBetweenMobileValue = getSpaceBetweenByAtrtribute(spaceBetweenMobileValue);
      let spaceBetweenTabletValue = el.getAttribute("space-between-tablet");
      spaceBetweenTabletValue = getSpaceBetweenByAtrtribute(spaceBetweenTabletValue);
      let spaceBetweenDesktopValue = el.getAttribute("space-between-desktop");
      spaceBetweenDesktopValue = getSpaceBetweenByAtrtribute(spaceBetweenDesktopValue);
      let slidesPerViewBaseValue = el.getAttribute("slides-per-view-base-value");
      slidesPerViewBaseValue = getslidesperViewByAttribute(slidesPerViewBaseValue);
      let slidesPerViewMobileValue = el.getAttribute(
        "slides-per-view-mobile-value"
      );
      slidesPerViewMobileValue = getslidesperViewByAttribute(slidesPerViewMobileValue);
      let slidesPerViewTabletValue = el.getAttribute(
        "slides-per-view-tablet-value"
      );
      slidesPerViewTabletValue = getslidesperViewByAttribute(slidesPerViewTabletValue);
      let slidesPerViewDesktopValue = el.getAttribute(
        "slides-per-view-desktop-value"
      );
      slidesPerViewDesktopValue = getslidesperViewByAttribute(slidesPerViewDesktopValue);
  ...
  var initHome = () => {
    ...
    const showSecondSlideOnHover = () => {
      layoutItemEl.forEach((el) => {
        ...
        if (window.screen.width <= 991 || window.screen.width <= 1024 && window.detectMobile() || window.screen.width <= 1100 || window.detectMobile() || window.mobileAndTabletCheck()) {
          console.log(window.detectMobile(), window.mobileAndTabletCheck());
          addClass();
        } else {
          removeClass();
